...
public class Card extends JLabel implements MouseListener{
	...
	public Card(Game game, int rank, Color color) {
		Random r=new Random();
		this.game = game;
		this.value = rank-1+r.nextInt(2)+(rank>=3?(r.nextInt(2)+2*(rank-2)):0);
		this.color = color;
		int i=0;
		// 设置购买
		...
		// 画好卡片
		try {
			this.img = ImageIO.read(new File(Common.cardImg[Common.getPosition(color)]+(new Random().nextInt(5)+1)+".BMP"));
			Graphics g = this.img.getGraphics();
			g.setColor(Color.black); g.setFont(Common.font_CardValue );
			if(this.value!=0)
			...
			for(i = 0; i < 5; i++) {
				if(coins[i]>0) {
					pos++;
					g.setColor(Common.getColor(i)); g.fillOval(3, CardImgHeight - pos * 30 + 5, 22, 22);
					g.setColor(i!=1? Color.white : Color.black); g.setFont(Common.font_CoinInCard );
	...
	public void paint(Graphics g) {
		...
		height = getHeight();
		g.drawImage(img.getScaledInstance(width, height, java.awt.Image.SCALE_SMOOTH), getWidth() / 2 - width / 2, 0, null);
		if(checked) {
	...
	public void mousePressed(MouseEvent arg0) {
		...
		if(checked) game.cardChecker = this;
		else game.cardChecker = null;
	}
	@Override
